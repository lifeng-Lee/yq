...
function readShareCode() {
  return new Promise(async resolve => {
    $.get({url: `http://api.yqqy.top/jd/getCode?active=jd_pet&num=${randomCount}`, 'timeout': 10000}, (err, resp, data) => {
      try {
        if (err) {
...
function submitCode() {
  return new Promise(async resolve => {
    $.get({url: `http://api.yqqy.top/jd/submit?active=jd_pet&code=${$.petInfo.shareCode}`, timeout: 10000}, (err, resp, data) => {
      try {
        if (err) {
          console.log(`${JSON.stringify(err)}`)
