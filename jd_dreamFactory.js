...
function readShareCode() {
  console.log(`开始`)
  return new Promise(async resolve => {
    $.get({url: `http://api.yqqy.top/jd/getCode?active=jx_factory&num=${randomCount}`, 'timeout': 10000}, (err, resp, data) => {
      try {
        if (err) {
          console.log(`${JSON.stringify(err)}`)
          console.log(`${$.name} API请求失败，请检查网路重试`)
...
function submitCode() {
  return new Promise(async resolve => {
    $.get({url: `http://api.yqqy.top/jd/submit?active=jx_factory&code=${myInviteCode}`, timeout: 10000}, (err, resp, data) => {
      try {
        if (err) {
          console.log(`${JSON.stringify(err)}`)
          console.log(`${$.name} API请求失败，请检查网路重试`)
...
function shareCodesFormat() {
  return new Promise(async resolve => {
    ...
    $.newShareCodes = ['Am4txzIGMET35yvcfE3Llg==',
                    'b4Iyz0Xn8gMbF5UFjN2d1Q==',
                        'pBQsgfu-jHVcl3VvDkO0RA==',
                        'DrItB4mZW6AwabGiqWqTLQ==',
                        '4N1ojB0zuHbWZeGTZ8b1uQ==',];
