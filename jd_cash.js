...
function readShareCode() {
  ...
  return new Promise(async resolve => {
    $.get({url: `http://api.yqqy.top/jd/getCode?active=jd_cash&num=${randomCount}`, 'timeout': 10000}, (err, resp, data) => {
      try {
        if (err) {
          console.log(`${JSON.stringify(err)}`)
          console.log(`${$.name} API请求失败，请检查网路重试`)
...
function submitCode() {
  return new Promise(async resolve => {
    $.get({url: `http://api.yqqy.top/jd/submit?active=jd_cash&code=${myInviteCode}`, timeout: 10000}, (err, resp, data) => {
      try {
        if (err) {
          console.log(`${JSON.stringify(err)}`)
          console.log(`${$.name} API请求失败，请检查网路重试`)
