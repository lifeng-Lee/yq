...
function readShareCode() {
  ...
  return new Promise(async resolve => {
    $.get({
      url: `http://api.yqqy.top/jd/getCode?active=jd_health&num=${randomCount}`,
      'timeout': 10000
    }, (err, resp, data) => {
      try {
        if (err) {
...
function submitCode() {
  return new Promise(async resolve => {
    $.get({url: `http://api.yqqy.top/jd/submit?active=jd_health&code=${myInviteCode}`, timeout: 10000}, (err, resp, data) => {
      try {
        if (err) {
          console.log(`${JSON.stringify(err)}`)
