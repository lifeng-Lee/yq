...
================Loon==============
[Script]
cron "21 3,8,11,14 * * *" script-path=https://raw.githubusercontent.com/Aaron-lv/sync/jd_scripts/jd_speed_sign.js,tag=京东极速版
...
async function signInit() {
  return new Promise(resolve => {
    $.get(taskUrl('speedSignInit', {
      ...
      "kernelPlatform": "RN",
      "inviterId":"Juk4L0oPs7TCoZ%2BmUPzWNoOn8bJjlhOf"
    }), async (err, resp, data) => {
      try {
        if (err) {
...
function invite2() {
  let t = +new Date()
  let inviterId = [
    "Juk4L0oPs7TCoZ%2BmUPzWNoOn8bJjlhOf",
